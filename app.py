    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            try:
                ...
                rid = None  # initialise here so poll block always has a binding
...
async def solve_hcaptcha_async(checkout_url: str, proxy: str | None = None) -> str | None:
    ...
    async with aiohttp.ClientSession(
        ...
        # ── 1. Extract actual sitekey from checkout page ────────────────────
        try:
        ...
        # ── 3. Poll for result (max 150 s at 5 s intervals) ────────────────
        for _ in range(30):
...
_RATE_LIMIT  = int(os.environ.get('RATE_LIMIT', '10'))    # requests per window per IP
